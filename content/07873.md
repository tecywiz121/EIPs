...
This EIP uses terminology from the [EIP-3540](./03540.md) which introduces the EOF format.
...
This mechanism complements `EOFCREATE` and `RETURNCODE` instructions from [EIP-7620](./07620.md), and thus all use cases of contract creation that are available in legacy EVM are enabled for EOF.
...
| `MAX_CODE_SIZE` | Defined as `24576` in [EIP-170](./00170.md) |
| `MAX_INITCODE_SIZE` | Defined as `2 * MAX_CODE_SIZE` in [EIP-3860](./03860.md) |

### Transaction Types

Introduce new transaction `InitcodeTransaction` (type `INITCODE_TX_TYPE`) which extends [EIP-1559](./01559.md) (type 2) transaction by adding a new field `initcodes: List[ByteList[MAX_INITCODE_SIZE], MAX_INITCODE_COUNT]`.
...
`initcodes` items data costs the same as calldata: transaction gas of an `InitcodeTransaction` is extended to include tokens in `initcodes` alongside tokens in `calldata`. Using the conventions from [EIP-7623](./07623.md), the transaction gas is calculated as:
...
Given the definitions from [EIP-2718](./02718.md) the `TransactionPayload` for an `InitcodeTransaction` is the RLP serialization of:
...
- behavior on `accessed_addresses` and address collision ([EIP-684](./00684.md) and [EIP-2929](./02929.md))
- EVM execution frame created for the `TXCREATE` initcode - memory, account context etc.
- nonce bumping of the account of newly created contract [EIP-161](./00161.md)
- balance checking and transfer for the creation endowment (`value` argument)
    
Introduce a new instruction on the same block number [EIP-3540](./03540.md) is activated on: `TXCREATE` (`0xed`).
...
- execute the container and deduct gas for execution. The 63/64th rule from [EIP-150](./00150.md) applies.
...
- a successful execution ends with initcode executing `RETURNCODE{deploy_container_index}(aux_data_offset, aux_data_size)` instruction (see [EIP-7620](./07620.md)). After that:
...
At the start of the block in which [EIP-3540](./03540.md) activates, set the code of `CREATOR_CONTRACT_ADDRESS` to `CREATOR_CONTRACT_BYTECODE`. `CREATOR_CONTRACT_ADDRESS` is not treated like a precompile. `CREATOR_CONTRACT_BYTECODE` corresponds to the following source code compiled to EOF:
...
This also makes [EIP-7698](./07698.md) (EOF - Creation transaction) no longer an essential requirement for deploying EOF contracts onto the chain. The EIP could be removed from EOFv1 and withdrawn.

### New address hashing scheme

`TXCREATE` uses the scheme `new_address = keccak256(0xff || sender || salt)[12:]`, same as `EOFCREATE` instruction. The decision whether to include initcontainer hash into salt is left to the `TXCREATE` caller. See [EIP-7620](./07620.md) for detailed rationale.
