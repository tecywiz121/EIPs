...
Introduce the ability to have several code sections in EOF-formatted ([EIP-3540](./03540.md)) bytecode, each one representing a separate subroutine/function. Two new opcodes,`CALLF` and `RETF`, are introduced to call and return from such a function. Dynamic jump instructions are disallowed.
...
[EIP-4200](./04200.md) introduces static jump instructions, which remove the need for *most* dynamic jump use cases, but not everything can be solved with them.
...
2. *Note:* EOF validation [EIP-5450](./05450.md) guarantees that operand stack has enough items to use as callee's inputs.
...
7. Pushes to return stack an item:
   ...
   *Note:* EOF validation [EIP-5450](./05450.md) guarantees there is always an instruction following `CALLF` (since terminating instruction or unconditional jump is required to be final one in the section), therefore `PC_post_instruction` always points to an instruction inside section bounds.
...
2. *Note:* EOF validation [EIP-5450](./05450.md) guarantees that operand stack has exact number of items to use as outputs.
3. Charges 3 gas.
...
In addition to container format validation rules above, we extend code section validation rules (as defined in [EIP-3670](./03670.md)).
...
3. Stack underflow check is not performed anymore. *Note:* EOF validation [EIP-5450](./05450.md) guarantees that it cannot happen at run-time.
