...
![FOCIL diagram](./assets/diagram_FOCIL.png)
...
- Same-slot: With FOCIL running in parallel with the block building process for `slot N+1` during `slot N`, the constraints imposed on `block B` for `slot N+1` can include transactions submitted during `slot N`. This represents a strict improvement over forward IL designs like [EIP-7547](../07547.md), where the forward property introduced a 1-slot delay.
