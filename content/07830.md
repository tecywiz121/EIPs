...
[EIP-170](./00170.md) specifies `MAX_CODE_SIZE` as 24576 bytes, and [EIP-3860](./03860.md) specifies `MAX_INITCODE_SIZE` as `2 * MAX_CODE_SIZE` (49152 bytes).
...
This increase still fits within the gas schedule, limiting the size to less than what gas limits allow.  In [EIP-170](./00170.md) the gas limit was first set "by setting the cap at a value slightly higher than what is feasible with current gas limits." At that time the gas limit had not exceeded 5M gas.
