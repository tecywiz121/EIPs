...
We define a new [EIP-2718](./02718.md) transaction type, where `TransactionType` is `WormholeTxType` and the `TransactionPayload` format is as follows:
...
* Use a transparent setup; ideally reuse the [EIP-4844](./04844.md) setup. Do not use Groth16 or other application-specific setups.
