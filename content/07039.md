...
This proposal (affectionateely known as SHADOW) is an alternative to [EIP-1193](./01193.md) for wallet discoevery in web browsers that requires no special peremissions. Web pages intending to open a connection to a wallet inject an `iframe` tag pointing at a well-known scheme. Communication between the page and the wallet uses the `postMessage` API.
