...
| Constant | Value |
| - | - |
| `EOF_MAGIC` | Defined as `0xEF00` in [EIP-3540](./03540.md) |
...
1. Intrinsic gas cost rules and limits defined in [EIP-3860](./03860.md) for creation transactions apply. The entire `data` of the transaction is used for these calculations.
...
6. Execute the container and deduct gas for execution.
    ...
    2. A successful execution ends with initcode executing `RETURNCODE{deploy_container_index}(aux_data_offset, aux_data_size)` instruction. After that:
        ...
        - set `state[new_address].code` to the updated deploy container (rules of [EIP-3541](./03541.md), prohibiting deployment of `code` starting with `EF` from creation transactions, do not apply in this case).
